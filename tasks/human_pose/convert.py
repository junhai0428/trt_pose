...
def torch2onnx(model_torch, model_onnx, input_size):
    ...
    '''input为输入模型图片的大小'''
    input = torch.zeros((1, 3, 480, 640)).cuda().half()
    ...
    print("transfer")
    torch.onnx.export(model, input, model_onnx, input_names=input_name, output_names=output_name, verbose=False)


if __name__ == "__main__":
    model_torch = "./epoch_249.pth"
    model_onnx = "./epoch_249_half.onnx"
